...
export const splitTasksRawSchema = z.object({
  ...
  tasksRaw: z
    .string()
    .describe(
      "Structured task list. Each task should be atomic and have clear completion criteria. Avoid overly simple tasks; simple modifications can be integrated with other tasks to avoid too many tasks. Example: [{name: 'Concise and clear task name, should clearly express the task purpose', description: 'Detailed task description, including implementation points, technical details, and acceptance criteria', implementationGuide: 'Specific implementation methods and steps for this task. Please refer to previous analysis results and provide concise pseudocode.', notes: 'Supplementary notes, special handling requirements, or implementation suggestions (optional)', dependencies: ['Full name of the prerequisite task this task depends on'], relatedFiles: [{path: 'File path', type: 'File type (TO_MODIFY: To Modify, REFERENCE: Reference Material, CREATE: To Create, DEPENDENCY: Dependent File, OTHER: Other)', description: 'File description', lineStart: 1, lineEnd: 100}], verificationCriteria: 'Verification standards and testing methods for this specific task'}, {name: 'Task 2', description: 'Task 2 description', implementationGuide: 'Task 2 implementation method', notes: 'Supplementary notes, special handling requirements, or implementation suggestions (optional)', dependencies: ['Task 1'], relatedFiles: [{path: 'File path', type: 'File type (TO_MODIFY: To Modify, REFERENCE: Reference Material, CREATE: To Create, DEPENDENCY: Dependent File, OTHER: Other)', description: 'File description', lineStart: 1, lineEnd: 100}], verificationCriteria: 'Verification standards and testing methods for this specific task'}]"
    ),
      ...
      relatedFiles: z
        .array(
          z.object({
            ...
            description: z.string().optional().describe("File description, used to explain the purpose and content of the file"),
            lineStart: z
              .number()
              .int()
              .positive()
      ...
      content: [
        {
          type: "text" as const,
          text:
            "tasksRaw parameter format error, please ensure the format is correct. Please try to fix the error. If the text is too long to fix easily, please call in batches to avoid issues with overly long messages that are difficult to correct. Error message: " +
